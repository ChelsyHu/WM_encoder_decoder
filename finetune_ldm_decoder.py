...
import os
os.environ["HF_ENDPOINT"] = "https://hf-mirror.com"   ##解决网络问题
...
def get_parser():
    ...
    aa("--ldm_config", type=str, default="LDM_configs/v2-inference.yaml", help="Path to the configuration file for the LDM model") 
    aa("--ldm_ckpt", type=str, default="  ", help="Path to the checkpoint file for the LDM model") 
...
def main(params):
    ...
    val_loader = utils.get_dataloader(params.val_dir, vqgan_transform, params.batch_size*2, num_imgs=100, shuffle=False, num_workers=4, collate_fn=None)
...
def train(data_loader: Iterable, optimizer: torch.optim.Optimizer, loss_w: Callable, loss_i: Callable, ldm_ae: AutoencoderKL, ldm_decoder:AutoencoderKL, msg_decoder: nn.Module, vqgan_to_imnet:nn.Module, key: torch.Tensor, params: argparse.Namespace):
    ...
    ldm_decoder.decoder.train()  ###对ldm的图像 decoder，进行训练微调
    ...
    for ii, imgs in enumerate(metric_logger.log_every(data_loader, params.log_freq, header)):
        ...
        # extract watermark
        decoded = msg_decoder(vqgan_to_imnet(imgs_w)) # b c h w -> b k     对被各种变换过的图片  进行message的提取
