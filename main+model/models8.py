...
class ResidualEncoder(nn.Module):
    ...
    def __init__(self, num_blocks, num_bits, channels, last_tanh=True):
        ...
        layers = [ConvBNRelu(num_bits, channels)]

        for _ in range(num_blocks-1):
            layer = ConvBNRelu(channels, channels)
            layers.append(layer)

        self.conv_bns = nn.Sequential(*layers) # b c h w 
        self.after_concat_layer = ConvBNRelu(channels + 3 + num_bits, channels)
    ...
    def forward(self, imgs, msgs):
        ...
        encoded_image = self.conv_bns(msgs) # b l h w  -> b c h w 
        im_w = self.final_layer(encoded_image)  # b 3 h w 
