...
class Encoder(nn.Module):
    ...
    def __init__(self, num_blocks, channel_of_watermark, channels, last_tanh=True ):
        ...
        self.after_concat_layer = ConvBNRelu(channels +1+ 3 + channel_of_watermark, channels)
    ...
    def forward(self, image, watermark):
        ...
        output: b 3 h w 
        """
        img= torch.cat([watermark, image, watermark], dim=1 )
        encoded_image=self.conv_bns(img)  # b 64 h w 

        concat = torch.cat( [watermark ,encoded_image, watermark, image], dim=1)  # b (channel_of_watermark+c+1+3) h w 
...
class Decoder(nn.Module):
    ...
    def __init__(self, num_blocks, channel_of_watermark, channels):
        ...
        layers1=[]
        layers1.append( nn.Conv2d(3, 3*2, kernel_size=4, stride=2, padding=1)  )  # b 4 h w   ->  b c 128 128  
        layers1.append( nn.BatchNorm2d(3*2) )  
        layers1.append( nn.ReLU(True)  )

        layers1.append( nn.Conv2d(3*2, 3*2, kernel_size=4, stride=2, padding=1)  )  # b 4 128 128    ->  b c 64 64 
        layers1.append( nn.BatchNorm2d(3*2) )  
        layers1.append( nn.ReLU(True)  )
        ...
        layers2.append( nn.ConvTranspose2d(3*2,3*2,kernel_size=4,stride=2,padding=1  ) )
        layers2.append(nn.BatchNorm2d( 3*2 ) )
        layers2.append( nn.ReLU(True))
...
class Decoder_Generator(nn.Module):
    def __init__(self,
        ...
        decoder_lis = [
            nn.ConvTranspose2d(32,16, kernel_size=4, stride=2, padding=1, bias=True),
            nn.InstanceNorm2d(16),
            nn.ReLU(True),
            # 16* 64*64
...
class EncoderDecoder(nn.Module):
    ...
    def forward(
        ...
        imgs_w = self.scaling_i * imgs + self.scaling_w * deltas_w # b 3 h w
        ...
        if eval_mode:     ## evaluation mode 
            ...
            wm2 = self.decoder(imgs_w)  ## b 3 h w 
