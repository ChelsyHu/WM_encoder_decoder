...
# os.environ['n_nodes']='1'
# os.environ['n_gpu_per_node']='2'
...
def get_parser():
    ...
    aa("--encoder", type=str, default="residual", help="Encoder type (Default: hidden, residual)")
    ...
    aa("--decoder", type=str, default="resnet50", help="Decoder type (Default: hidden, resnet50)")
    ...
    aa("--lambda_w", type=float, default=40.0, help="Weight of the watermark loss. (Default: 1.0)")
    aa("--lambda_i", type=float, default=1, help="Weight of the image loss. (Default: 0.0)")
    aa("--lambda_latents", type=float, default=1, help="Weight of the image latents(default: 0.0 )" )
    ...
    #aa('--n_gpu_per_node', default=1, type=int)
...
def main(params):
    ...
    elif params.encoder =='residual':
        encoder =  models.ResidualEncoder(
            num_blocks=params.encoder_depth, num_bits=params.num_bits, 
    ...
    if params.decoder == 'hidden':
        ...
        decoder=models.CustomResNet(num_classes=params.num_bits)
    else:
    ...
    if params.dist: 
        ...
        encoder_decoder = nn.parallel.DistributedDataParallel(encoder_decoder)


        # Build optimizer and scheduler
    optim_params = utils.parse_params(params.optimizer)
    lr_mult = params.batch_size * utils.get_world_size() / 512.0 /5
