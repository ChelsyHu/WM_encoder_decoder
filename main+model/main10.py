...
def main(params):
    ...
    if params.dist: 
        ...
        # 如果VAE模型包含批量归一化层，将其转换为同步批量归一化
        vae = nn.SyncBatchNorm.convert_sync_batchnorm(vae)
        # 使用DistributedDataParallel包装VAE模型
        vae = nn.parallel.DistributedDataParallel(vae, device_ids=[params.local_rank])
    ...
    optim_params['lr'] = 0.0005 # lr_mult * optim_params['lr'] if 'lr' in optim_params else lr_mult * 1e-3
    to_optim = [*encoder.parameters(), *decoder.parameters()]
    #optimizer = utils.build_optimizer(model_params=to_optim, **optim_params)
    optimizer = torch.optim.AdamW(to_optim, lr=optim_params['lr']  )
    scheduler = None  #
