...
def get_parser():
    ...
    aa("--lambda_i", type=float, default=0.05, help="Weight of the image loss. (Default: 0.0)")
...
def main(params):
    ...
    # pipeline_image2image.vae = AutoencoderTiny.from_pretrained("/ssd-sata1/hqq/stable_signature/taesd", torch_dtype=torch.float16).to('cuda')
    ...
    if params.dist: 
        ...
        encoder_decoder = nn.parallel.DistributedDataParallel(encoder_decoder, device_ids=[params.local_rank]) # device_ids=None
    ...
    optim_params = utils.parse_params(params.optimizer)
    lr_mult = params.batch_size * utils.get_world_size() / 512.0 *8 
    optim_params['lr'] = 0.000001 #lr_mult * optim_params['lr'] if 'lr' in optim_params else lr_mult * 1e-3  # 0.001 -> 0.0005 
    to_optim = [*encoder.parameters(), *decoder.parameters()]
    optimizer = utils.build_optimizer(model_params=to_optim, **optim_params)
    #optimizer = torch.optim.AdamW(to_optim, lr=1e-3  )
    scheduler = None  #utils.build_lr_scheduler(optimizer=optimizer, **utils.parse_params(params.scheduler)) if params.scheduler is not None else None
    ...
    transform = transforms.Compose([
        transforms.Grayscale(num_output_channels=1),  # 确保图像是灰度的
        transforms.Resize((params.img_size, params.img_size)),  # 调整图片大小
        transforms.ToTensor(),  # 将PIL图像转换为张量
    ])
    # 读取图像
    image_path = 'imgs/text_image/Chelsy_Hu1.png'  # 替换为你的图像路径
    image = Image.open(image_path)
    # 应用转换
    tensor_image = transform(image)

    # 确保张量是浮点类型，通常用于训练
    tensor_name_image = tensor_image.float()
    ...
    for epoch in range(start_epoch, params.epochs):
        ...
        train_stats = train_one_epoch(encoder_decoder, train_loader, optimizer, scheduler, epoch, params,vae,tensor_name_image)
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()}, 'epoch': epoch}

        if epoch % params.eval_freq == 0:
            val_stats = eval_one_epoch(encoder_decoder, val_loader, epoch, params, vae, tensor_name_image)
...
def train_one_epoch(encoder_decoder: models.EncoderDecoder, loader, optimizer, scheduler, epoch, params,vae, tensor_name_image):
    """
    ...
    # if  epoch >=300 & epoch < 321:
    #     encoder_decoder.train()
    # elif epoch >=321  & epoch < 351:
    #     encoder_decoder.encoder.eval()
    #     encoder_decoder.decoder.train()
    # elif epoch >=351 & epoch <380:
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        # 创建一个全黑的张量
        latents_tensor = tensor_name_image.repeat(imgs.shape[0],imgs.shape[1],1,1 ).to(device)
        ...
        loss = params.lambda_w*loss_w + params.lambda_i*loss_i + params.lambda_latents_b*loss_latents_b
