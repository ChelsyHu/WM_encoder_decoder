...
def get_parser():
    ...
    aa("--lambda_i", type=float, default=0, help="Weight of the image loss. (Default: 0.0)")
    aa("--lambda_gen", type=float, default=0, help="Weight of the generated_image loss. (Default: 1.0)")
...
def main(params):
    ...
    encoder_decoder = models.EncoderDecoder(encoder, attenuation, data_aug, decoder, 
        params.scale_channels, params.scaling_i, params.scaling_w, params.num_bits, params.redundancy, device)   ##################focus
    ...
    lr_mult = params.batch_size * utils.get_world_size() / 512.0 * 35  ## 0.0109
    ...
    best_bit_acc = 0
    #encoder_decoder=encoder_decoder.half()
...
def train_one_epoch(encoder_decoder: models.EncoderDecoder, loader, optimizer, scheduler, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        #fts, (imgs_generation, imgs_w, imgs_w_generation) = encoder_decoder(imgs, msgs)
        fts, (imgs_w, imgs_w_generation) = encoder_decoder(imgs, msgs)
        ...
        #loss_gen = image_loss(imgs_generation, imgs_w_generation, loss_type= params.loss_i_type)  # b c h w -> 1
        
        #loss = params.lambda_w*loss_w + params.lambda_i*loss_i + params.lambda_gen * loss_gen 
        ...
        # if ((it + 1) % accum_iter == 0) or (it + 1 == len(loader)):
        #     optimizer.step()
        #     optimizer.zero_grad()
        ...
        optimizer.step()
                ...
                # 打印参数更新情况
        # for name, param in encoder_decoder.encoder.named_parameters():
        #     #print(f"Parameter {name}")
        #     if param.data.norm() == 0:
        #         print(f"Parameter {name} has not been updated.")

        for name, param in encoder_decoder.encoder.named_parameters():
            print(f"Layer: {name}, requires_grad: {param.requires_grad}, grad: {param.grad}")
        ...
        if epoch % params.saveimg_freq == 0 and it == 0 and utils.is_main_process():
            ...
            save_image(utils_img.unnormalize_img(imgs_w_generation), os.path.join(params.output_dir, 'train',f'{epoch:03}_{it:03}_train_w_gen.png'), nrow=8)
            #save_image(utils_img.unnormalize_img(imgs_generation), os.path.join(params.output_dir, 'train',f'{epoch:03}_{it:03}_train_gen.png'), nrow=8)
...
def eval_one_epoch(encoder_decoder: models.EncoderDecoder, loader, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        loss_w = message_loss(fts, msgs, m=params.loss_margin, loss_type=params.loss_w_type) # b k -> 1
        loss_i = image_loss(imgs_w, imgs, loss_type=params.loss_i_type) # b c h w -> 1
        #loss_gen = image_loss(imgs_generation, imgs_w_generation, loss_type= params.loss_i_type)
        ...
        if epoch % params.saveimg_freq == 0 and it == 0 and utils.is_main_process():
            ...
            save_image(utils_img.unnormalize_img(imgs_w_generation), os.path.join(params.output_dir, 'validation',f'{epoch:03}_{it:03}_val_w_gen.png'), nrow=8)
            #save_image(utils_img.unnormalize_img(imgs_generation), os.path.join(params.output_dir, 'validation',f'{epoch:03}_{it:03}_val_gen.png'), nrow=8)
