...
import utils
import utils_img
import models_newpipe as models
import attenuations
...
def get_parser():
    ...
    aa("--decoder", type=str, default="Resnet50", help="Decoder type (Default: hidden),choice: hidden, Resnet50")
    aa("--decoder_depth", type=int, default=4, help="Number of blocks in the decoder (Default: 4)")
    aa("--decoder_channels", type=int, default=32, help="Depth of blocks in the decoder (Default: 32)")
...
# 定义一个线性增加学习率的函数
def linear_lr_scheduler(optimizer, initial_lr, final_lr, num_iter):
    def lr_lambda(epoch):
        return (final_lr / initial_lr) ** (epoch / num_iter)
...
def main(params):
    ...
    elif params.decoder == 'Resnet50':
        decoder = models.Resnet50(n_class=params.num_bits)
    else:
    ...
    encoder_decoder = models.EncoderDecoder(encoder, attenuation, data_aug, decoder, 
        params.scale_channels, params.scaling_i, params.scaling_w, params.num_bits, params.redundancy, vae)   ##################focus
    ...
    # lr_mult = params.batch_size * utils.get_world_size() / 512.0 /32
    # lr = lr_mult * optim_params['lr'] if 'lr' in optim_params else lr_mult * 1e-3
    #lr = 0.02
    ...
    for param_group in optimizer.param_groups:
        param_group['lr'] =  4e-4 #lr #optim_params['lr'] 
    ...
    for epoch in range(start_epoch, params.epochs):
        ...
        train_stats = train_one_epoch(encoder_decoder, train_loader, optimizer,  epoch, params)
...
def train_one_epoch(encoder_decoder: models.EncoderDecoder, loader, optimizer, epoch, params):
    """
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        fts, (imgs_w) = encoder_decoder(imgs, msgs)
        ...
        # Loss_anti_tamper= - image_loss( vae.encode(imgs_w).latents, vae.encode(imgs).latents )

        loss = params.lambda_w*loss_w + params.lambda_i*loss_i # +loss_anti_tamper
        ...
        # for name, param in encoder_decoder.decoder.named_parameters():
        #     print(f'Layer: {name}, Gradient: {param.grad}')
    ...
    # # 绘制学习率与损失的关系图
    # plt.plot(lrs, losses)
    # plt.xscale('log')  # 学习率使用对数尺度
    # plt.xlabel('Learning Rate')
    # plt.ylabel('Loss')
    # plt.title('Learning Rate Range Test')
    # #plt.show()
    # plt.savefig('learning_rate_range_test4.png')
...
def eval_one_epoch(encoder_decoder: models.EncoderDecoder, loader, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        fts, (imgs_w) = encoder_decoder(imgs, msgs, eval_mode=True)
