...
import utils
import utils_img
import models_pipeimg2img as models
import attenuations
...
def get_parser():
    ...
    aa("--prompts",type=str, default='a photo', help="the prompts for image generation")
    aa("--pretrained_model_name_or_path",type=str, default='/hdd2/hqq/stable_signature/stable-diffusion-2-1-base')

    group = parser.add_argument_group('Distributed training parameters')
    aa('--debug_slurm', action='store_true')
    aa('--local_rank', default=-1, type=int)   ##important in the training with muti-GPUs,  置为0 当多GPU的时候(不， 置为device的一个值)
...
def main(params):
    ...
    prompts=[params.prompts]*params.batch_size


    # Create encoder/decoder
    encoder_decoder = models.EncoderDecoder(encoder, attenuation, data_aug, decoder, 
        params.scale_channels, params.scaling_i, params.scaling_w, params.num_bits, params.redundancy,pipeline_image2image, prompts)   ##################focus
...
def train_one_epoch(encoder_decoder: models.EncoderDecoder, loader, optimizer, scheduler, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        loss_gen = image_loss(imgs, imgs_w_generation, loss_type=params.loss_i_type)
        ...
        if epoch % params.saveimg_freq == 0 and it == 0 and utils.is_main_process():
            ...
            save_image(utils_img.unnormalize_img(imgs_w_generation), os.path.join(params.output_dir, 'train',f'{epoch:03}_{it:03}_train_gen.png'), nrow=8)
...
def eval_one_epoch(encoder_decoder: models.EncoderDecoder, loader, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        fts, (imgs_w, imgs_w_generation) = encoder_decoder(imgs, msgs, eval_mode=True)
        ...
        if epoch % params.saveimg_freq == 0 and it == 0 and utils.is_main_process():
            ...
            save_image(utils_img.unnormalize_img(imgs_w_generation), os.path.join(params.output_dir, 'validation',f'{epoch:03}_{it:03}_val_gen.png'), nrow=8)
