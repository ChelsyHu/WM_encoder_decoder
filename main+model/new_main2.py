...
def get_parser():
    ...
    aa("--output_dir", type=str, default="output/output_img/3", help="Output directory for logs and images (Default: /output)")
    ...
    aa("--decoder", type=str, default="Decoder_Generator", help="Decoder type (Default: Decoder. Choices:Decoder, Decoder_Generator)")
    ...
    aa("--scaling_w", type=float, default=0.4, help="Scaling of the watermark signal. (Default: 1.0)")
    aa("--scaling_i", type=float, default=0.6, help="Scaling of the original image. (Default: 1.0)")
    ...
    aa("--lr",  type=float, default= 1e-5, help="Learning rate (Default: 1e-3)"  )

    group = parser.add_argument_group('Loader parameters')
    aa("--batch_size", type=int, default=8, help="Batch size. (Default: 16)")
    aa("--batch_size_eval", type=int, default=8, help="Batch size. (Default: 128)")
...
def train_one_epoch(encoder_decoder: new_models.EncoderDecoder, loader, optimizer, scheduler, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        wm2, imgs_w = encoder_decoder(imgs, watermark_b)  ##, imgs_aug 可能输出    ## input: b 3 h w , b 1 h w 
