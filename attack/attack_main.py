...
def get_parser():
    ...
    aa("--model_path", type=str, defalut='/hdd2/hqq/stable_signature/stable-diffusion-2-1-base', help='Model path.'  )
...
def main(params):
    ...
    lr_mult = params.batch_size * utils.get_world_size() / 512.0 /8 * 2
    ...
    vae = AutoencoderKL.from_pretrained("/hdd2/hqq/stable_signature/stable-diffusion-2-1-base/vae",variant='fp16')
    vae.eval()
    vae.to(device)
    ...
    for epoch in range(start_epoch, params.epochs):
        ...
        train_stats = train_one_epoch(encoder_decoder, train_loader, optimizer, scheduler, epoch, params, vae)
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()}, 'epoch': epoch}
        
        if epoch % params.eval_freq == 0:
            val_stats = eval_one_epoch(encoder_decoder, val_loader, epoch, params, vae)
...
def vector_loss(imgs_w, imgs, vae):
    imgs_w_embedding = vae.encode(imgs_w).latent_dist.sample() ##水印图片的嵌入
    imgs_embedding = vae.encode(imgs).latent_dist.sample()

    imgs_black = torch.zeros_like(imgs_w)
    imgs_white = torch.ones_like(imgs_w) 

    imgs_black_embedding=vae.encode(imgs_black).latent_dist.sample()
    imgs_white_embedding=vae.encode(imgs_white).latent_dist.sample()

    a= F.cosine_similarity(imgs_w_embedding,imgs_black_embedding, dim = -1).mean()
    b= F.cosine_similarity(imgs_w_embedding,imgs_white_embedding, dim = -1).mean()
    c= F.cosine_similarity(imgs_w_embedding,imgs_embedding, dim = -1).mean()

    return a+b-c 

def train_one_epoch(encoder_decoder: models.EncoderDecoder, loader, optimizer, scheduler, epoch, params,vae ):
    """
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        loss_v = vector_loss(imgs_w, imgs, vae)

        loss = params.lambda_w*loss_w + params.lambda_i*loss_i + params.lambda_v*loss_v
...
@torch.no_grad()
def eval_one_epoch(encoder_decoder: models.EncoderDecoder, loader, epoch, params, vae):
    """
    One epoch of eval.
    """
    vae.eval()
