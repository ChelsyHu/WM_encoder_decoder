...
def get_parser():
    ...
    aa("--output_dir", type=str, default="ckpts", help="Output directory for logs and images (Default: /output)")
    ...
    aa("--lambda_w", type=float, default=50.0, help="Weight of the watermark loss. (Default: 1.0)")
    ...
    aa('--local_rank', default=0, type=int)   ##important in the training with muti-GPUs,  置为0 当多GPU的时候.单GPU置为-1
...
def main(params):
    ...
    optim_params['lr'] = 0.0000001 #lr_mult * optim_params['lr'] if 'lr' in optim_params else lr_mult * 1e-3
    ...
    test_stats = test_one_epoch(encoder_decoder, test_loader, params, vae)
    ...
    if utils.is_main_process():
        with (Path(params.output_dir) / "log_test_all.txt").open("a") as f:
...
@torch.no_grad()
def test_one_epoch(encoder_decoder: models.EncoderDecoder, loader,  params,vae):
    """
    One epoch of test.
    """
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        for img_pixel, img_name in zip(imgs_w, names):
            ...
            save_image(utils_img.unnormalize_img(img_pixel),save_path, nrow=1)
