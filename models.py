...
class LatentEncoder(nn.Module):
    ...
    """
    def __init__(
            self, num_blocks, channels, last_tanh=True):
        ...
        layers.append(  ConvBNRelu(channels, 3) )
    ...
    def forward(self, imgs):
        ...
        deltas_w = self.conv_bns(imgs) # b c h w

        if self.last_tanh:
            deltas_w = self.tanh(deltas_w)
        ...
        #     heatmaps = self.attenuation.heatmaps(imgs) # b 1 h w
        #     deltas_w = deltas_w * heatmaps # # b c h w * b 1 h w -> b c h w
        imgs_w =1.0* deltas_w  +  1.0* imgs # b c h w
